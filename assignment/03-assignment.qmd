...
Authenticate with the Qualtrics API using your UO API key and data center ID. Instructions for locating these in your Qualtrics account settings are posted on Canvas.
...
Pull responses from the shared course practice survey (survey ID posted on Canvas) using `fetch_survey()`. Print the first few rows and run `glimpse()` on the result.

### Task G.3

Compare the API output to the manually exported CSV version of the same survey (also on Canvas). Note at least two differences in column names, data types, or structure. What would you need to do to make them match?

### Task G.4

Write a short reflection (~half a page, as a comment block in your script) answering: When would you use the API rather than a manual export in your own research? What are the tradeoffs in terms of effort, reliability, and reproducibility?

**Submission:** Add your code and reflection to the same `.R` file, clearly marked with a comment block: `# === GRADUATE EXTENSION ===`.
