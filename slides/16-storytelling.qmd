...
::: {.notes}
Welcome back — reminder that Monday was Memorial Day, so it's been a full week since they were last in class (Session 15 on joins). Today is Session 16: Storytelling with Data. Assignment 7 (the joins + missing data assignment) was due Sunday, and Assignment 8 (Storytelling Report) is assigned today. Also distribute the APA figure formatting handout at the start of class. Structure: ~25 min lecture, ~15 min pair coding, ~20 min lecture part 2, ~10 min work time.
:::
...
::: {.notes}
The end-of-deck exercise has two parts. First, ~5 minutes per published figure to critique with a partner, then debrief. Second, the A8 head-start: students pick a dataset and start a minimal Quarto report. Be explicit that A8 is NOT the final project — students conflate them every year. If time is short, skip the second figure and go straight to the A8 scaffolding so they can ask rendering questions while you're in the room.
:::
...
4. **One** exploratory visualization with a caption (apply today's storytelling principles)
5. **One** inline-code statistic in the narrative
