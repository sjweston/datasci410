...
## Solution: Missing data patterns {background-color="#e67e22"}

```{r}
#| echo: true
#| eval: false
# 1. NAs in every column at once — across() applies sum(is.na(.))
#    to each variable so we don't have to name them one by one.
therapy_survey |>
  summarize(
    across(everything(), ~sum(is.na(.x)))
  )

# 2. Complete cases — drop_na() with no arguments removes any row
#    that has an NA anywhere. nrow() then counts what's left.
therapy_survey |>
  drop_na() |>
  nrow()

# 3. Drop only rows missing followup — pass the column name to drop_na().
therapy_survey |>
  drop_na(followup_depression)

# 4. Completion rate — !is.na() flips the logic to "has a value",
#    and mean() of a TRUE/FALSE vector gives the proportion.
therapy_survey |>
  summarize(
...
## Solution: Longitudinal missing data {background-color="#e67e22"}

```{r}
#| echo: true
#| eval: false
# 1. Make implicit missing explicit. complete() generates every
#    participant x wave combination and fills in NA where data
#    is absent — surfacing dropout that was hidden in the rows.
long_complete <- longitudinal_study |>
  complete(participant, wave)

long_complete

# 2. Count assessments per person — !is.na() is TRUE when a
#    score exists, and sum() counts those TRUEs.
long_complete |>
  group_by(participant) |>
  summarize(
    n_assessments = sum(!is.na(depression))
...
# 3. Percentage who completed all 3 waves — a two-step summarize:
#    first count per participant, then take the mean of the
#    logical "n_assessments == 3" to get a proportion.
long_complete |>
  group_by(participant) |>
  summarize(
    n_assessments = sum(!is.na(depression))
...
# 4. Reshape wide for a per-participant view, then compute change.
#    pivot_wider() gives one row per person with wave_1/2/3 columns;
#    filter() keeps only those with both wave 1 and wave 3 so the
