...
```{r}
# Data export with combined columns
messy_data <- tibble(
  id = 1:3,
...
## Creating an APA-style summary table
...
```{r}
# Before pivoting: 3 rows
wide_scores

# After pivoting: 9 rows (3 students × 3 semesters)
wide_scores |>
  pivot_longer(-student, names_to = "semester", values_to = "score")
```

Check: `nrow(original) × ncol(pivoted_columns) = nrow(result)`
