...
:::: {.columns}
::: {.column width="40%"}
![](https://r4ds.hadley.nz/diagrams/EDA-boxplot.png)
:::
...
## Violin plots: See the distribution
...
## Even better: Show the actual data
...
## Another approach: Frequency polygons

```{r}
#| output-location: slide
ggplot(therapy_data, aes(x = depression_post, color = condition)) +
  geom_freqpoly(binwidth = 2) +
  labs(
    title = "Distribution of depression scores by condition",
...
| Plot type | Best for |
|-----------|----------|
| **Boxplot** | Quick comparison of medians and spread |
| **Violin** | Seeing the shape of distributions |
| **Violin + points** | Small to medium datasets, show individual cases |
| **Frequency polygon** | Comparing distributions across many groups |
...
2. **Different plot types** for different variable combinations:
   - Categorical + continuous: boxplot, violin, frequency polygon
   - Categorical + categorical: bar charts, tiles, geom_count()
   - Continuous + continuous: scatterplot, 2D bins, hex
3. **Watch for overplotting** — use alpha, jitter, or binning
4. **Always visualize first** before computing correlations
5. **Patterns suggest but don't prove** causation
