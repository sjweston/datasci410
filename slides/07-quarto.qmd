...
::: {.notes}
Session 7 (Week 4, Mon Apr 20). Quarto was moved earlier in the curriculum so students use it for all assignments from A3 onward. Assignment 3 (Tidying, Import & Quarto) is assigned today. Structure: ~25 min lecture, ~15 min pair coding, ~20 min lecture part 2, ~10 min work time. Open with the callback to Session 1 — this is the payoff for the replication crisis discussion.
:::
...
```{{r}}
# Load data
data <- read_csv(here("data", "depression_data.csv"))
...
::: {.notes}
This is the main hands-on exercise. Give students 10 minutes. Walk around and help — the most common issues will be: YAML formatting errors (missing dashes, wrong indentation), forgetting to close code chunks, and render failures because of missing packages or objects. Remind them to create a new .qmd file from the RStudio menu (File > New File > Quarto Document), not by hand. They don't need to worry about inline code yet — that's coming after the break.
:::
...
3. Create a code chunk that loads `tidyverse` and a dataset (try `mpg` — it's built into ggplot2, just type `mpg`)
4. Create a visualization in its own chunk
5. Render to HTML

**Time: 10 minutes**

## Solution: Create a mini-report {background-color="#e67e22"}

```{r}
#| echo: true
#| eval: false

# Save this as a .qmd file, then click Render:
...
# ## Introduction
# This report examines fuel efficiency using the `mpg` dataset.
# We explore the relationship between engine size and highway economy.
...
````{markdown}
`r round(mean(depression_scores), 2)`
````

Without `round()`, you might get 15 decimal places. Always round inline results.

::: {.notes}
`round()` is the formatting function they'll use most. Emphasize: if your inline code prints something like 19.0000000000001, you forgot to round. The second argument is the number of decimal places. This is a habit to build now — every inline number should be rounded.
:::

## A note on output formats

Quarto can also render to **PDF** and **Word** — but these require installing additional (free) software.

For this course, we'll stick with **HTML documents**. They're easy to create, easy to share, and work on any device.

::: {.notes}
Keep this brief. If students ask: PDF requires a LaTeX installation (TinyTeX), Word output works out of the box but has less control over formatting. Both are free but add setup complexity we don't need right now. HTML is the simplest path and what they'll submit for assignments.
:::
...
Table code goes **inside a code chunk**, just like plots. The `knitr::kable()` function turns a data frame into a nicely formatted table:
...
4. **Use `here()` for file paths** — `here("data", "file.csv")` always finds the project root
...
Use `here()` to build file paths: `read_csv(here("data", "survey_data.csv"))` always works, no matter where RStudio thinks the working directory is.

::: {.notes}
This connects back to Session 1's filing cabinet metaphor. A well-organized project folder means anyone (including future you) can find the data, the code, and the output. The .qmd file goes in the root because it references everything else with relative paths. Emphasize `here()` — it always finds paths relative to the project root, avoiding the "it works on my laptop but not yours" problem. They learned about `here` in Session 6 (Data Import); this is where it pays off.
:::
...
- **Tables** — `knitr::kable()` inside code chunks for formatted tables
- **`here()`** for file paths that always work
...
- Start working on **Assignment 3** — your first Quarto submission
...
See you Wednesday for Layers & Aesthetics — we'll build on your ggplot skills with new geoms and customization options.

# End-of-deck exercise {background-color="#e67e22"}

::: {.notes}
This exercise is a direct head start on Assignment 3 — A3 is the first Quarto submission, so students should leave class with a working `.qmd` skeleton that imports both A3 data files. Give ~10 minutes. They probably won't finish, and that's fine — priority is a successful first render. Walk around and help with rendering errors, especially YAML indentation and missing package loads, which will be frequent. Remind them this same workflow is used for every assignment from A3 onward.
:::

## Head start: Assignment 3

Assignment 3 is your **first Quarto submission**. Start scaffolding it now:

1. Create a new `.qmd` with a YAML header (title, author, `format: html`)
2. Add sections: Introduction, Data, Results
3. Setup chunk that loads `tidyverse`, `readxl`, and `here` (`#| include: false`)
4. Import the A3 data files using `here()` (e.g., `read_csv(here("data", "survey_data.csv"))`)
5. `glimpse()` each in its own chunk
6. Report one summary statistic with **inline code** in the narrative
7. Render to HTML — fix any errors before you leave class

::: {.callout-tip}
This is the workflow you'll use for every assignment from A3 onward.
:::
...
# --- SOLUTION EXAMPLE ---
# Save this as a .qmd file (e.g., assignment-3.qmd):
...
# ## Introduction
# This report imports and explores two datasets related to [topic].
...
# ```{r}
# #| label: load-demographics
# demographics <- read_excel(here("data", "demographics.xlsx"),
#                            sheet = 2)
# glimpse(demographics)
# ```

# ## Results
# The survey included `r nrow(survey_data)` participants.
# The mean age was `r round(mean(demographics$age, na.rm = TRUE), 1)` years.
```
