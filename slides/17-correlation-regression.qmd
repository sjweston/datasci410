...
tidy(model) |>
  mutate(
    ...
    p = format_p_apa(p.value)
  ) |>
  select(term, estimate, p, meaning) |>
  knitr::kable(digits = 2, col.names = c("Term", "Estimate", "p", "In plain language"))
```
...
## R-squared: How much can we predict?
...
**R-squared** is the proportion of variation in y that is predictable from x.

- R² = 0 → x tells us nothing about y
- R² = 1 → x perfectly predicts y
- R² = `r round(glance(model)$r.squared, 2)` → knowing social media use predicts about `r round(glance(model)$r.squared * 100)`% of the variation in anxiety scores

. . .

The other `r 100 - round(glance(model)$r.squared * 100)`% reflects everything else that varies across people (genetics, life events, personality, measurement noise).

::: {.notes}
Be deliberate about language here: "predicts" and "is associated with," not "explains" or "causes." Students just heard "correlation does not equal causation" a few slides ago, and "explains" sounds causal even though stats textbooks use it loosely. Reframe low R² as a feature, not a bug: in psychology, single predictors rarely capture most of the variation in human behavior, which is why researchers use multiple regression (preview of what's next). Students sometimes think low R² means the model is "bad" -- reframe it as "one piece of a bigger puzzle."
:::
...
```{r}
# Is it significant?
cor.test(class_survey$sleep_hrs, class_survey$stress, use = "complete.obs")
```
...
## Does the relationship differ by group?

`group_by()` + `summarize()` works the same with `cor()` as with any other summary:

```{r}
class_survey |>
  group_by(chronotype) |>
  summarize(
    r = cor(sleep_hrs, stress, use = "complete.obs"),
    n = n()
  )
```
...
One pipeline, one correlation per group. Same skill you've been using all term — just a new summary function.

::: {.notes}
This slide ties together two big ideas: group_by/summarize from Session 3 and correlation from today. Emphasize that cor() is just another summary statistic, like mean() or sd(). Ask: "Does the sleep–stress relationship look the same across chronotypes, or does it differ?" Let students react to the numbers. If the groups differ noticeably, that's a great moment to mention that moderation (whether a relationship depends on a third variable) is where this kind of analysis goes next — but don't go deeper than naming it.
:::
