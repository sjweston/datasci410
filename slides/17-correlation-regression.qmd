...
::: {.notes}
This is individual work time (~10 minutes). The exercise mirrors the anxiety analysis but uses depression (phq9_total) instead, so students can apply the same workflow independently. Circulate and check that students can interpret their slope in a sentence -- that's the hardest part. Encourage students who finish early to try the bonus (broom::tidy table). Remind them that this exercise is also good practice for the final project if they have continuous variables.
:::
