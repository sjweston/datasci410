...
A participant rates these three items from the Rosenberg Self-Esteem
Scale on a 1–5 scale (1 = Strongly Disagree, 5 = Strongly Agree):

| # | Item | Resp. |
|---|------|-------|
| 1 | I feel that I am a person of worth, at least on an equal plane with others. | 4 |
| 2 | I feel that I have a number of good qualities. | 3 |
| 3 | All in all, I am inclined to feel that I am a failure. | 2 |

: {tbl-colwidths="[5,85,10]"}

. . .

What's their self-esteem score so far?

. . .

If you said **9**, you forgot that item 3 is reverse-coded — a `2`
should become `(5 + 1) - 2 = 4`, making the real total **11**. Mistakes
like this happen in published papers, and they live in how you handle
data types.

::: {.notes}
Spend ~3 minutes here. Let students answer out loud before revealing the catch — most will say 9 for the total (or 3.0 if they took the mean). The correct values are 11 for the total and 3.67 for the mean, once item 3 is reversed. The point is that the wording of item 3 ("I am inclined to feel that I am a failure") is going in the opposite direction of items 1 and 2, so a high response actually means *low* self-esteem. The student has to notice that themselves. Mention that reverse-coding errors are genuinely common in published work.
:::
...
## Solution: Recode responses {background-color="#e67e22"}
...
attention_data <- attention_data |>
  mutate(
    passed = attention_check == 4,
    # if_else with the `missing` argument handles NA in one shot
    status = if_else(attention_check == 4, "Passed", "Failed",
                     missing = "No response"),
    # same logic, using the passed column we just created
    status2 = if_else(passed, "Passed", "Failed",
                      missing = "No response"),
    # case_when names each condition explicitly — check NA first
    status3 = case_when(
      is.na(attention_check) ~ "No response",
      attention_check == 4   ~ "Passed",
      .default               = "Failed"
...
# Proportion who passed
mean(attention_data$passed, na.rm = TRUE)
```
...
::: {.notes}
Quickly mention the conceptual side: dropping NAs is the easy default, but it is not always the right call. In psychology, missingness is often not random — people skip sensitive questions. We will come back to this in Session 14.
:::
...
By default, `mutate()` works **column-wise** — `sum()` and `mean()`
inside it see entire columns, not individual rows.

. . .

For scale scoring we need the opposite: combine values *across*
columns *within* a single participant.

. . .

`rowwise()` switches `mutate()` into **row-by-row mode**:

```{r}
#| eval: false
data |>
  rowwise() |>     # switch to row-by-row mode
  mutate(...) |>   # now sees one row at a time
  ungroup()        # switch back when done
```

::: {.notes}
This is a conceptual setup slide — do not run any code. Walk through the metaphor: by default mutate is "looking at all participants at once for one column"; rowwise flips it to "looking at all columns for one participant in turn." Pause on the three-step pattern (rowwise → mutate → ungroup) and warn that forgetting ungroup() leaves the data row-grouped, which makes later operations slow and surprising. Tell students they will see this pattern over and over today.
:::

## Why rowwise() matters

Without it, `sum()` collapses across the whole column — the same number
for every row:
...
::: {.notes}
Run this and let the output speak for itself. Every row gets the same total — the sum of all values across all participants and items. This is the "wrong" version that motivates rowwise() on the next slide. Ask: "Why is the total identical for every participant?"
:::

## Option 2: Sum with na.rm + rowwise()

Now combine `sum(..., na.rm = TRUE)` with `rowwise()`:
...
::: {.notes}
This is the payoff. Each participant now gets their own total, and NA values are dropped instead of poisoning the sum. Compare side-by-side with the previous slide if students need to see the difference. Remind them again to ungroup() — this is the most common bug in scale-scoring code.
:::
...
See you Monday for strings and factors!

::: {.notes}
End on this takeaway. Remind students that Assignment 5 is due before next class, and the reading for next session is R4DS Ch 14 (just sections 14.1-14.3) and Ch 16 on factors. Preview that next class covers strings and factors — the other two data types from the opening slide. The PHQ-9 practice exercise that follows is for them to try on their own time as a bridge to the next assignment.
:::
...
::: {.notes}
This is a take-home exercise — students do not need to finish it in class. It ties together everything from today: rowwise(), sum with na.rm, if_else for the missingness threshold, and case_when for severity categories. The solution uses c_across(), which is a nice shortcut. If a student asks during office hours, watch for the case_when cutoffs — they often get the boundary conditions wrong (e.g., using < instead of <=).
:::

---

## Solution: Score the PHQ-9 {background-color="#e67e22"}
...
phq_data |>
  rowwise() |>
  mutate(
    # c_across() lets us select multiple columns rowwise
    n_answered = sum(!is.na(c_across(phq1:phq9))),

    # Two thresholds: keep total only if enough items answered
    total_7 = if_else(n_answered >= 7,
                      sum(c_across(phq1:phq9), na.rm = TRUE),
                      NA_real_),
    total_9 = if_else(n_answered >= 9,
                      sum(c_across(phq1:phq9), na.rm = TRUE),
                      NA_real_)
  ) |>
  ungroup() |>
  mutate(
    # Check is.na first — comparisons with NA return NA and fall through
    category = case_when(
      is.na(total_9) ~ NA,
      total_9 <  5   ~ "Minimal",
      total_9 < 10   ~ "Mild",
      total_9 < 15   ~ "Moderate",
      total_9 < 20   ~ "Moderately Severe",
      .default       = "Severe"   # catches 20–27
    )
  )
```
