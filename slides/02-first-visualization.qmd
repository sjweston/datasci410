...
ggplot(mpg, aes(displ, hwy)) + geom_point()  # x and y are first args
```
...
Using the `mpg` dataset, create a scatterplot that:

1. Shows city mpg (`cty`) vs highway mpg (`hwy`)
2. Colors points by fuel type (`fl`)
3. Adds a linear trend line
4. Has appropriate labels
5. Uses `theme_bw()`
