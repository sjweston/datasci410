...
::: {.notes}
Point out that full_join() gives you the most complete picture — nobody is dropped. But it also means you may have rows with lots of NAs — exactly the kind of explicit missingness we discussed last session. Good moment to call back to drop_na(), replace_na(), and the "make implicit missing explicit" idea: full_join() does the same job, just for combining tables.
:::
...
::: {.notes}
This is the payoff for the compound-key idea from earlier in the deck — when one column does not uniquely identify a row, you join on multiple columns. Walk through what would happen if we used `by = "id"` alone: participant 1's Time 1 depression would match both of participant 1's anxiety rows (Time 1 and Time 2), doubling the output. The vector `c("id", "time")` tells dplyr to require BOTH columns to match. This is the syntax students will use any time longitudinal data is spread across files.
:::
...
::: {.notes}
Circulate during the exercise. The most common mistake is using inner_join() instead of left_join(), which silently drops participant 5 in step 1. If pairs finish early, ask them to figure out which participants are missing followup data and whether missingness is related to condition. That bridges to the filtering joins section. When time is up, walk through the solution and emphasize step 3 — the sum(is.na()) pattern is the same one they used last session for missing data.
:::

## Solution: Combine study data {background-color="#e67e22"}
...
::: {.notes}
Many-to-many joins are the sneakiest mistake students will make with real data. The join runs without an error — but the row count explodes. Always sanity-check: if you joined an 8-row table with a 4-row table and got 16 rows, something is off. The fix is almost always to add another column to the `by` argument so the match becomes unique. This is the same compound-key idea from the longitudinal join earlier in the deck.
:::
...
## Solution: Longitudinal study {background-color="#e67e22"}
...
::: {.notes}
End strong on this takeaway. Remind students that Assignment 7 (Joins and Missing Data) is due Sunday May 24 — both halves of the assignment are now fair game. Note that Memorial Day is the following Monday so the next class is Wednesday May 27 (Storytelling with Data). The final project draft was due today, so they should now be working on cleaning and analyzing their own data with all the tools they've learned.
:::
